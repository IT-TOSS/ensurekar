...
const PlansSection = ({ planData = defaultPlanData, page = "page1", serviceName = "Income Tax Return Filing" }: PlansSectionProps) => {
  ...
  const handlePlanSelection = (planId: string, planName: string, price: string) => {
    try {
      ...
      const selectedPlan = {
        id: `${serviceName.toLowerCase().replace(/\s+/g, "-")}-plan-${planId}`,
      ...
      <div className="max-md:flex flex max-md:flex-col flex-wrap md:flex max-md:gap-[16px] items-start justify-around">
        ...
        <div className="flex md:hidden justify-between items-center mx-auto">
          {gotPlansData.map((plan) => (
            <p
              className={`text-[16px] px-4 py-1 text-[#8095A7] font-medium cursor-pointer ${plan.planName === selectPlan ? "border-b-2 border-[#022B50] font-semibold" : ""
                }`}
              key={plan.id}
        ...
        <div className="flex justify-center items-start flex-wrap mx-auto">
          {gotPlansData.map((plan) => (
            <div
              key={plan.id}
              className={`p-4 flex m-3 group relative flex-col gap-[10px] mx-4 md:w-[460px] my-[25px] md:mt-0 hover:bg-yellow-400 hover:border-[#007AFF] border-[1px] rounded-md shadow-[0px_0px_10px_rgba(104,104,104,0.08)] cursor-pointer transition delay-150 duration-500 ${plan.planName === selectPlan ? "md:block" : "max-md:hidden md:block"
                }`}
            >
