...
const Page = () => {
  ...
  // Delete plan - Uses the delete_package.php endpoint with ID in URL
  const deletePlanFromAPI = async (id: string) => {
    try {
      console.log("🗑️ [Delete] Deleting plan with ID:", id)
      
      // Use the delete_package.php endpoint with ID in URL query parameter
      const deleteUrl = `https://edueye.co.in/ensurekar/existing-site/delete_package.php?id=${id}`
      ...
      if (!response.ok) {
        ...
        try {
          const errorData = await response.json()
          errorText = errorData.error || errorData.details || JSON.stringify(errorData)
          console.error("🗑️ [Delete] API error response (JSON):", errorData)
        } catch (e) {
          try {
            errorText = await response.text()
            console.error("🗑️ [Delete] API error response (Text):", errorText)
          } catch (e2) {
      ...
      const result = await response.json().catch(async () => {
        ...
        return { success: true, message: text || "Plan deleted successfully" }
      })
      ...
      console.log("✅ [Delete] Plan deleted successfully:", result)
      return result
    } catch (error) {
      console.error("❌ [Delete] Failed to delete plan:", error)
      throw error
    }
  }
